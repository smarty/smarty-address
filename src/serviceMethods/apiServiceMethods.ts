...
export const fetchAddressSuggestions: ServiceMethod = async ({state, services, utils}, searchString) => {
	try {
		const suggestions = await utils.getAutocompleteApiResults(searchString, state.apiKey, state.autocompleteApiUrl);
...
export const fetchSecondaryAddressSuggestions: ServiceMethod = async ({state, services, utils}, {selectedAddress, searchString}) => {
	try {
		const suggestions = await utils.getAutocompleteApiResults(searchString, state.apiKey, state.autocompleteApiUrl, selectedAddress);
